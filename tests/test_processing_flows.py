...
from odoo.addons.payment_razorpay.controllers.main import FaisapayController
from odoo.addons.payment_razorpay.tests.common import FaisapayCommon
...
class TestProcessingFlows(RazorpayCommon, PaymentHttpCommon):
    ...
    def test_redirect_notification_triggers_processing(self):
        ...
        url = self._build_url(f'{FaisapayController._return_url}?reference={self.reference}')
        with patch(
            'odoo.addons.payment_faisapay.controllers.main.FaisapayController'
    ...
    @mute_logger('odoo.addons.payment_faisapay.controllers.main')
    def test_webhook_notification_triggers_processing(self):
        """ Test that receiving a valid webhook notification triggers the processing of the
        notification data. """
        self._create_transaction('direct')
        url = self._build_url(FaisapayController._webhook_url)
        with patch(
            'odoo.addons.payment_razorpay.controllers.main.FaisapayController.'
            '_verify_notification_signature'
        ), patch(
            'odoo.addons.payment.models.payment_transaction.PaymentTransaction'
            '._handle_notification_data'
    ...
    def test_accept_webhook_notification_with_valid_signature(self):
        ...
        with patch(
            ...
            '._faisapay_calculate_signature', return_value='valid_signature'
        ):
            self._assert_does_not_raise(
                Forbidden,
                FaisapayController._verify_notification_signature,
